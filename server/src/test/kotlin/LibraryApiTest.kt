...
import com.schwarzdigitale.auth.LoginRequest
import com.schwarzdigitale.auth.LoginResponse
...
class LibraryApiTest {
    ...
    @Test
    fun testRegisterCustomer() = testApplication {
    ...
    fun testLoginSuccess() = testApplication {
        ...
        assertEquals(testEmail, loginResponse.customer.email)
    }
    
    @Test
    fun testLoginWithWrongPassword() = testApplication {
        val client = createClient {
            install(ContentNegotiation) {
                json(Json { ignoreUnknownKeys = true })
            }
    ...
    @Test
    fun testCreateCategoryWithoutAuth() = testApplication {
        val client = createClient {
            install(ContentNegotiation) {
                json(Json { ignoreUnknownKeys = true })
            }
    ...
    @Test
    fun testCreateCategoryWithAuth() = testApplication {
        val client = createClient {
            install(ContentNegotiation) {
                json(Json { ignoreUnknownKeys = true })
            }
    ...
    @Test
    fun testGetCategoriesPublic() = testApplication {
        val client = createClient {
            install(ContentNegotiation) {
                json(Json { ignoreUnknownKeys = true })
            }
    ...
    fun testCreateAndGetBook() = testApplication {
        ...
        // Create book
        val bookResponse = client.post("/books") {
            contentType(ContentType.Application.Json)
            bearerAuth(token)
            setBody(BookCreateRequest(
                title = "The Hobbit",
    ...
    @Test
    fun testDeleteBookWithAuth() = testApplication {
        val client = createClient {
            install(ContentNegotiation) {
                json(Json { ignoreUnknownKeys = true })
            }
        ...
        // Delete the book
        val deleteResponse = client.delete("/books/${bookResponse.id}") {
            bearerAuth(token)
        }
